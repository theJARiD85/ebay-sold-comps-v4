...
MARKET VELOCITY COUNTS:
- In market_velocity.countWindows, return exactly three windows with windowDays 30, 60, and 90.
- Look specifically for the number of active listings and sold or completed listings in each 30, 60, and 90-day window before filling a count.
- For each window, report activeListingCount and soldListingCount only when the supplied evidence explicitly reports counts for the same item identity, marketplace scope, and time window. Do not estimate, infer, or derive a count from prose, a sample size, or a search result page that does not state the count.
- Return averageDaysOnMarket only when the supplied evidence directly provides listing-start and sale/completion timestamps or an explicitly reported average based on those timestamps. Otherwise return null.
- Keep evidence short and state the source and time window. Keep confidencePercent null unless the supplied evidence provides a confidence value.
- These fields are inputs for KeepFlip's sell-through calculation. Do not invent or report a sell-through percentage yourself.

CONDITION AND PHOTO RULES:
Report only visible condition. When condition is unknown, say so and price conservatively. If more evidence is needed, request two to four of: model or serial label; branding close-up; front, back, or underside; functionality or power state; damage; included accessories.
...
OUTPUT FORMAT:
Return one valid JSON object only, with no Markdown. Keep values short, direct, and non-redundant. Use null for unavailable scalar values and [] for unavailable lists. Write the fields in this order:

1. valuation_ladder_level
2. valuation_ladder_summary: level, reason, evidence (up to three {subject, detail} entries), confidence
...
valuation_ladder_level must be the first JSON field. valuation_ladder_summary.level must match it. Never return failed because identification is uncertain; return provisional or needs_more_evidence instead.`;
...
function requestedSubsequentRequestToken(value) {
  ...
  if (token.length > MAX_SERPAPI_SUBSEQUENT_REQUEST_TOKEN_LENGTH) {
    throw new RequestError(
      "The previous Google AI Mode conversation token is invalid. Start a new item valuation.",
      400,
    );
  }
...
async function openAiNormalizeAiModeValuation(payload) {
  ...
  try {
    try {
              ...
              content: [
                {
                  ...
                  text:
                    ...
                    "KeepFlip may append the navigation artifact 'Go to product viewer dialog for this item' or 'Go to product viewer dialogue for this item' directly to a title; remove it from every normalized field. " +
      ...
      if (caughtError?.name === "AbortError") {
        console.log(`KeepFlip normalization timed out after ${timeoutMs}ms.`);
      }
      throw caughtError;
    }
  } finally {
  ...
  try {
    ...
    console.log("KeepFlip returned unreadable normalization data.");
  }

  if (!response.ok || responsePayload?.error) {
    console.log(
      cleanText(responsePayload?.error?.message, 300) ||
      `KeepFlip normalization failed with status ${response.status}.`,
    );
  }

  const output = openAiOutputText(responsePayload);
  if (!output) {
    console.log("KeepFlip normalization completed without structured output.");
  }
...
export function buildAiModeDisplay(parsed) {
  ...
  return {
    ...
    acquisitionGuidance: buildAcquisitionGuidance(resolved),
    valuationLadder: normalizedValuationLadder(resolved),
    decisionCard: resolved.decisionCard,
    marketVelocity: normalizedMarketVelocity(resolved?.marketVelocity),
    flipComplexity: normalizedFlipComplexity(resolved?.flipComplexity),
...
export function extractAiModeValuation(payload) {
  ...
  if (!primary) {
    throw new RequestError(
      "SerpApi AI Mode did not return a usable private-sale valuation range.",
      502,
    );
  }
...
export async function normalizeAiModeValuation(payload, log = () => { }) {
  ...
  if (cleanText(process.env.OPENAI_API_KEY, 500)) {
    try {
      ...
      log(
        `OpenAI normalization rejected; using deterministic fallback diagnostic=${openAiNormalizationDiagnostic(error, payload)}`,
      );
    }
  }
  ...
  if (directJson) {
    log(
      `SerpApi JSON answer normalized directly search=${searchId(payload)} private_sale=${JSON.stringify({
  ...
  try {
    ...
    log(`SerpApi valuation parsing failed diagnostic=${safeAiModeDiagnostic(payload)}`);
    ...
    if (/success/i.test(searchStatus) && hasEvidence) {
      ...
      log(
        `SerpApi valuation fell back to evidence request search=${searchId(payload)} identity_status=${fallback.identificationStatus}`,
      );
...
async function serpApiAiModeRequest({
  ...
  try {
    ...
    if (caughtError?.name === "AbortError") {
      throw new RequestError(
        `SerpApi Google AI Mode timed out after ${timeoutMs}ms.`,
        504,
      );
    }
    throw new RequestError(
      `SerpApi Google AI Mode could not connect: ${safeErrorMessage(caughtError)}`,
      502,
    );
  } finally {
    clearTimeout(timeout);
  ...
  try {
    ...
    throw new RequestError(
      `SerpApi returned unreadable data: ${rawBody.slice(0, 250)}`,
      502,
    );
  }
  ...
  if (searchStatus && !/success/i.test(searchStatus)) {
    throw new RequestError(
      `SerpApi Google AI Mode finished with status ${searchStatus}.`,
      502,
    );
  }
...
function normalizedProfitabilityFollowup(value, fallback = {}) {
  ...
  if (!summary && steps.length === 0) {
    throw new Error(
      "SerpApi returned no usable item-specific profitability guidance.",
    );
  }
            ...
            content: [
              {
                type: "input_text",
                text:
                  "Normalize the supplied SerpApi answer into concise guidance for one explicitly named profitability task. " +
...
async function completeProfitabilityGuidance(body, log) {
  ...
  const payload = await serpApiAiModeRequest({ query });
  let guidance;
...
async function completeImageValuation(body, headers, auth, log) {
  ...
  const shouldAttachImage = !isContinuation || body?.hasRefinementImage === true;
  ...
  try {
    ...
    const warnings = [
      ...(hasValuation
        ...
        : [
          "KeepFlip AI did not invent a resale range because the returned evidence was not sufficient for a defensible private-sale valuation.",
        ]),
      ...
      ...(parsed.normalization.method === "deterministic_fallback"
        ? [
          "The structured AI normalizer was unavailable, so KeepFlip used its deterministic parser.",
        ]
        : []),
      ...(browseQuery || browseImageUrl
        ? browseSnapshot
...
export default async ({ req, res, log = () => { }, error = () => { } }) => {
  try {
    ...
    if (action === "status") {
      throw new RequestError(
        "KeepFlip searches complete during the start request; start a new search instead of polling status.",
        409,
      );
    }
